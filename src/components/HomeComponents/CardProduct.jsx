...
const CardProduct = ({dataProduk}) => {
  ...
  return (
    <>
      <Link
        href={`/product/${dataProduk.product_id}`}
        className="w-72 h-[450px] bg-white rounded-md py-3 mb-5 flex flex-col justify-between border-2 shadow-md"
      >
        ...
        <div className="px-2 flex flex-col gap-y-1">
          ...
          <div className="flex items-center text-xs text-[#3F3F46]">
            ...
            <p className="ms-1 font-medium text-sm">{dataProduk.user?.kota}</p>
          </div>
        </div>
      </Link>
    </>
